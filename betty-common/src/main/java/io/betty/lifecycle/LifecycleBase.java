...
import io.betty.util.ExceptionUtils;
import io.betty.util.InternalSlf4JLoggerFactory;
import io.betty.util.StringManager;
...
public abstract class LifecycleBase implements Lifecycle {
    ...
    private static final Logger logger = InternalSlf4JLoggerFactory.getLogger(LifecycleBase.class);
    ...
    public final synchronized void start() throws LifecycleException {
        ...
        if (LifecycleState.STARTING_PREP.equals(state) || LifecycleState.STARTING.equals(state) ||
            ...
            if (logger.isDebugEnabled()) {
                ...
                logger.debug(sm.getString("lifecycleBase.alreadyStarted", getName()), e);
            } else if (logger.isInfoEnabled()) {
                logger.info(sm.getString("lifecycleBase.alreadyStarted", getName()));
            }
    ...
    public final synchronized void stop() throws LifecycleException {
        ...
        if (LifecycleState.STOPPING_PREP.equals(state) || LifecycleState.STOPPING.equals(state) ||
            ...
            if (logger.isDebugEnabled()) {
                Exception e = new LifecycleException();
                logger.debug(sm.getString("lifecycleBase.alreadyStopped", getName()), e);
            } else if (logger.isInfoEnabled()) {
                logger.info(sm.getString("lifecycleBase.alreadyStopped", getName()));
            }
        ...
        try {
            if (state.equals(LifecycleState.FAILED)) {
                ...
                // fired
                fireLifecycleEvent(LifecycleState.STOPPING_PREP, null);
            } else {
    ...
    public final synchronized void destroy() throws LifecycleException {
        if (LifecycleState.FAILED.equals(state)) {
            try {
                ...
                logger.warn(sm.getString("lifecycleBase.destroyStopFail", getName()), e);
            }
        }
        ...
        if (LifecycleState.DESTROYING.equals(state) || LifecycleState.DESTROYED.equals(state)) {
            if (logger.isDebugEnabled()) {
                Exception e = new LifecycleException();
                logger.debug(sm.getString("lifecycleBase.alreadyDestroyed", getName()), e);
            } else if (logger.isInfoEnabled() && !(this instanceof Lifecycle.SingleUse)) {
                // Rather than have every component that might need to call
                // destroy() check for SingleUse, don't log an info message if
                // multiple calls are made to destroy()
                logger.info(sm.getString("lifecycleBase.alreadyDestroyed", getName()));
            }
    ...
    private synchronized void setStateInternal(LifecycleState state, Object data, boolean check)
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(sm.getString("lifecycleBase.setState", getName(), state));
        }
...
//        String lifecycleEvent = state.getLifecycleEvent();
    ...
    private void invalidTransition(String type) throws LifecycleException {
        String msg = sm.getString("lifecycleBase.invalidTransition", type, getName(), state);
